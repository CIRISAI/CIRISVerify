...
| `bindings/python` | Released | **ciris-verify 5.1.0** on PyPI with platform wheels, wallet signing, named key storage, **`jcs_canonicalize` module-level binding (#61)**, **`resolve_role_authority` + `verify_partner_record_quorum` module-level bindings (#65)**, **`verify_accord_custody_attestation` module-level binding (#91)** |
...
./target/release/ciris-verify accord holder --key-id accord-eric-moore-primary --module .../libykcs11.so --key-label "..." --pin
# Coordinator builds the canonical entrenched-family envelope (3 primaries, roster order)
./target/release/ciris-verify accord family-envelope --member accord-eric-moore-primary --member accord-eric-kudzin-primary --member accord-haley-bradley-primary --out family.json
# Each founder co-signs on THEIR token (no human signs another's key)
./target/release/ciris-verify accord co-sign --envelope family.json --key-id accord-eric-moore-primary --module .../libykcs11.so --key-label "..." --pin --out cosign-moore.json
# Coordinator assembles → verifies the accord's 2/3 quorum of DISTINCT keys, then writes the genesis → outbox
./target/release/ciris-verify accord assemble --envelope family.json --cosign cosign-moore.json --cosign cosign-kudzin.json --cosign cosign-bradley.json
...
./target/release/ciris-verify accord holder --key-id accord-eric-moore-primary --module .../libykcs11.so --key-label "..." --pin --portable-usb /media/usb-A1
...
./target/release/ciris-verify accord invoke --key-id accord-eric-moore-primary --roster roster.json --kind constitutional --invocation-id halt-2026-06-19 --payload-sha256 <hex> --module .../libykcs11.so --key-label "..." --pin
# Holder B lists pending objects (flags the ones whose roster they belong to)
./target/release/ciris-verify accord list --mine accord-eric-kudzin-primary
# Holder B concurs → 2/2, quorum met
./target/release/ciris-verify accord concur --object <invocation.json> --key-id accord-eric-kudzin-primary --module .../libykcs11.so --key-label "..." --pin
```
