...
**Last updated:** 2026-08-03 (v12.6.0 "TPM EK vendor anchors baked at their real provenance tier" — **AV-51 over-trusted anchor provenance**: 40 TPM vendor roots ship as `CommunityAggregated`, not `VendorOfficial`, because they are sourced from Microsoft's `TrustedTPM.cab` rather than from the vendors; no TPM vendor operates a machine-fetchable root endpoint, and the one community aggregation that claims to (`1id-com/tpm-manufacturer-cas`) contains **zero certificates**. Only self-signed roots baked (never intermediates), self-signatures cryptographically verified, one expired root excluded; full sourcing record in [`TPM_ANCHOR_PROVENANCE.md`](TPM_ANCHOR_PROVENANCE.md). A `VendorOfficial` demand yields nothing from the TPM set, asserted by test (#199, #227). Prior: 2026-08-02 (v11.0.0 "CC 3.1.2.1 v2 canonical bytes / device-attestation legs / constrained anchors" — **AV-50 delimiter injection in provenance canonical bytes**: v1 built `key=value\n` preimages over attacker-influenceable free text (`source` = `direct:{url}`, `target`, `signer_identity`) with no newline guard, so a crafted value could forge field boundaries; CC 3.1.2.1 v2 replaces it with `sha256(JCS({…}))`, immune by construction (**wire break** — invalidates every v1 signature; the accord-invocation preimage is deliberately exempt per CC so the baked #107 genesis stays stable). **AV-49 unconstrained trust anchor**: CoTS-shaped store resolves on (purpose, environment) together, fail-closed on omission (#227). **AV-47 lifted device attestation** / **AV-48 over-claimed hardware class**: Android Key Attestation + Apple App Attest walk to pinned vendor roots and bind the attested key + challenge; over-claims are refuted, absence is never a failure (#199). **AV-46 relayed build-attestation bundle**: presenter binding (#181). Prior: 2026-06-18 (v6.0 "self-at-login device identity" — AV-43 CEG outbox / AV-44 software-PQC-seed honest-boundary / AV-45 surviving-key revocation; YubiKey PIV §5 row. Prior v5.11.0 "CEG §19 / §19.7 holonomic" — `holonomic` substrate verifiers; §19 cross-impl-proven (Edge v4.1.2), §19.7 promoted to **1.0** (Edge v4.3.0 reproduced Verify-authored vectors), RC16 `EjectAggregatedTierOnly` verdict, RC15 §19.1 Merkle frozen (no RFC-6962 prefix). See [`HOLONOMIC_SUBSTRATE.md`](HOLONOMIC_SUBSTRATE.md). Prior v5.7.0 "CEG 1.0-RC7 / hybrid-required" — **PQC half MANDATORY at every federation-tier admission gate** (#75, RC7 §10.1.5.1.1): `threshold`/`provenance`/license gates reject classical-only ("hybrid-pending") signatures via `HybridPolicy::RequireHybrid`, closing the F-AV-14 "buggy verifier accepts old-classical-only" gap (see §3.2 AV-8 and `FEDERATION_THREAT_MODEL.md` F-AV-14); partnership seven-member set (#76); `infra:*`/`agency:*` delegation scope split makes §1.3 "infrastructure must not have agency" wire-checkable (#77). v5.6.0 "security-audit remediation" — license-signature gate (#72), real TPM sealing (#73), fail-secure keygen (#74), RNS `destination_hash` recompute (#28), producer signing (#63). Prior v5.0.0 "CEG 1.0 / Agent 3.0 substrate" — RNG SP 800-90B startup health-check + fail-secure (AV-39 closed), `boundary_degraded` orthogonal to `hardware_trust_degraded`, JCS/RFC-8785 cross-impl signing bytes, `key_grant` wrap_algorithm v2 (X25519+ML-KEM-768) at-rest PQC, `doc_integrity` hybrid-signed artifact integrity + CODEOWNERS two-person-rule on threat-model docs, `infrastructure_community` M-of-N trust-root substrate)))
...
| AV-46 | Relayed build-attestation bundle (impostor presents another node's evidence) | **Presenter binding** — the bundle is bound-hybrid-signed by the presenting node and its `attesting_key_id` MUST equal the pinned presenter member, with pubkeys resolved from the caller's directory and never from the object (`build_attestation_bundle::verify_build_attestation_bundle`); carried manifest is hash-committed and re-rooted independently to the accord anchors | Optional RFC 6962 inclusion proof must reconstruct **and** carry *this* manifest as its leaf — a valid proof for an unrelated leaf does not count | ✓ Mitigated v10.7.0. **Honest boundary: this does NOT prove remote execution** (unprovable) — it proves an attributable, falsifiable assertion over independently-rooted evidence | CIRISVerify#181 |
| AV-47 | **Lifted device attestation** — a genuine device's genuine attestation replayed under an attacker's key | The attested key MUST equal the caller-pinned key (`AttestedKeyMismatch` / `KeyIdMismatch`). Without this the chain walk is theatre: the chain is real, the presenter is not | Anti-replay via the challenge binding — Android `attestationChallenge`, Apple `nonce = sha256(authData ‖ sha256(challenge))` | ✓ Mitigated v10.8.0 (Android) / v11.0.0 (Apple); both driven end-to-end off mock-vendor-root chains, not unit tests alone | CIRISVerify#199 |
| AV-48 | Over-claimed hardware class (peer asserts stronger custody than it holds) | `refutes(claimed_class)` fires iff the peer claimed *stronger* custody than the chain measured (StrongBox-claim on a TEE key; production-claim on a development App Attest) | Verify emits **measurements**, consumers gate; absence of an attestation is never a failure and a weak-but-honest measurement is never refuted | ✓ Mitigated v10.8.0/v11.0.0. Weight refutations heavily, passes lightly, absence not at all — a *pass* is producible by any holder of a genuine chain | CIRISVerify#199 |
| AV-49 | Unconstrained trust anchor (a pinned root usable for any purpose/environment) | **CoTS-shaped constrained store** (`trust_anchor_store`, `draft-ietf-rats-concise-ta-stores`): `resolve(purpose, environment)` returns an anchor only when **both** match, so a Yubico PIV root cannot satisfy an Android lookup and a key-attestation root cannot validate a TLS cert | Fail-closed on omission — an empty `purposes`/`environments` list matches **nothing**, never a wildcard; baked anchors are digest-pinned and checked on every load | ✓ Mitigated v10.9.0–v10.11.0. Verify is the first Rust CoTS implementation | CIRISVerify#227 |
| AV-50 | **Delimiter injection in provenance canonical bytes** — free text carrying `\n` forges field boundaries in a concatenated preimage | v1 built `key=value\n` preimages over attacker-influenceable free text (`source` = `direct:{url}`, `target`, `signer_identity`) with **no newline guard**, so two logically different manifests could produce ambiguous canonical bytes. **v2 (CC 3.1.2.1) replaces it with `sha256(JCS({…}))`** — structure lives in the encoding, not in delimiters, so escaping is unnecessary by construction | CC 2.6.1.1 omit-vs-materialize (`valid_until` absent, not an empty-string sentinel); v2 golden vectors pin the exact JCS form for cross-impl agreement | ✓ Mitigated v11.0.0 (**wire break** — invalidates every v1 signature). Accord-invocation bytes deliberately exempt per CC: no attacker-controlled free text, and genesis-critical bytes must stay stable | CIRISVerify#231, #200 |
| AV-51 | **Over-trusted anchor provenance** — an anchor sourced from a third-party aggregation is consumed as though it were vendor-official | `AnchorProvenance` records how each anchor was *actually* sourced (`CallerSupplied` < `CommunityAggregated` < `VendorOfficial` < `HardwareValidated`), and `resolve_x509_min_provenance(purpose, environment, min)` lets a caller refuse anything below its bar. The 40 baked TPM EK roots ship as **`CommunityAggregated`** because they come from Microsoft's `TrustedTPM.cab` — Microsoft curates, but is a *third party* to Infineon/Nuvoton/ST/Atmel/NationZ/Qualcomm, and none were cross-confirmed against the vendors the way both Google roots were against two independent Google sources | Sourcing is written down and auditable (`docs/TPM_ANCHOR_PROVENANCE.md`: four routes attempted, three dead ends); every baked anchor is DER-digest-pinned and checked on load; only self-signed roots are baked (never intermediates), self-signatures verified cryptographically rather than name-matched, and an expired root was excluded | ✓ Mitigated v12.3.0 (tier) / v12.6.0 (first aggregated anchors ship under it). A test asserts a `VendorOfficial` demand yields **nothing** from the TPM set, so the tier cannot quietly stop meaning something | CIRISVerify#199, #227 |
...
| AV-51 | Trust Anchors | TPM vendor roots are third-party-aggregated (`CommunityAggregated`), not vendor-official; no TPM vendor publishes a machine-fetchable root endpoint |
